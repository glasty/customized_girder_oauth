...
class Custom4(ProviderBase):
    def getClientIdSetting(self):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_ID)

    def getClientSecretSetting(self):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_SECRET)

    @classmethod
    def getClientAuthUrl(cls):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_AUTH_URL)

    def getClientTokenUrl(self):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_TOKEN_URL)

    @classmethod
    def getClientScope(cls):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_SCOPE)
    
    @classmethod
    def getClientButtonColor(cls):
        color = Setting().get(PluginSettings.CUSTOM4_CLIENT_BUTTON_COLOR)
        return color.split(';')

    @classmethod
    def getClientIconUrl(cls):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_ICON_URL)

    @classmethod
    def getClientName(cls):
        return Setting().get(PluginSettings.CUSTOM4_CLIENT_NAME)
